...
namespace Obi
{
    ...
    public class ObiSoftbody : ObiActor, IShapeMatchingConstraintsUser
    {
        ...
        public override Matrix4x4 Teleport(Vector3 position, Quaternion rotation)
        {
            if (!isLoaded)
                return Matrix4x4.identity;

            Matrix4x4 offset = base.Teleport(position, rotation);
        ...
        public override void Interpolate(float simulatedTime, float substepTime)
        {
            var sc = solver.GetConstraintsByType(Oni.ConstraintType.ShapeMatching) as ObiConstraints<ObiShapeMatchingConstraintsBatch>;
            ...
            base.Interpolate(simulatedTime, substepTime);
        }
