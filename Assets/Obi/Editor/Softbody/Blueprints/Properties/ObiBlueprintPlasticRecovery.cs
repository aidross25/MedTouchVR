namespace Obi
{
    public class ObiBlueprintPlasticRecovery : ObiBlueprintFloatProperty
    {

        public ObiBlueprintPlasticRecovery(ObiActorBlueprintEditor editor) : base(editor, 0)
        {
        ...
        public override float Get(int index)
        {
            ...
            foreach (var batch in constraints.batches)
            {
                for (int i = batch.constraintCount - 1; i >= 0; --i)
                {
                    ...
                    {
                        return batch.materialParameters[i * 5 + 3];
                    }
                }
            }
            return 0;
        ...
        public override void Set(int index, float value)
        {
            ...
            foreach (var batch in constraints.batches)
            {
                for (int i = batch.constraintCount - 1; i >= 0; --i)
                {
                    ...
                    if (firstParticle == index)
                    {
                        batch.materialParameters[i * 5 + 3] = value;
